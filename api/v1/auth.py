...
from core.admin.admin_security import has_role, parse_roles, serialize_roles
# 初始化APIRouter
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[Dict[str, Any]]:
    ...
    user_data = {
        ...
        "roles": parse_roles(user_row["roles"]),
        "rolePermission": parse_roles(user_row["rolePermission"]),
        "menuPermission": parse_roles(user_row["menuPermission"]),
...
async def require_login(current_user: Optional[Dict[str, Any]] = Depends(get_current_user)) -> Dict[str, Any]:
    if not current_user:
        raise HTTPException(status_code=401, detail="未登录")
    return current_user


async def require_admin(current_user: Dict[str, Any] = Depends(require_login)) -> Dict[str, Any]:
...
async def register(req: RegisterDTO):
    ...
    try:
        ...
        role_permission = serialize_roles([])
        menu_permission = serialize_roles([])
        tenant_id = "default"
