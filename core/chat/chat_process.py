...
from core.knowledge.knowledge_queue import KnowledgeIngestionQueue
...
from core.chat.emotion_recognition import create_sentiment_chain
from core.chat.tool import SQLiteChatMessageHistory
...
class ChatProcess:
    ...
    async def lifespan(self: FastAPI):
        # 在应用启动时执行的代码
        print("应用正在启动...")
        # 导入 api.v1.chat 模块，以便能够设置其全局变量
        import api.v1.chat
        queue = None
        try:
            self.state.start_time = time.time()
            # （1）初始化模型和嵌入模型（使用辅助api)
            api.v1.chat.model = ChatOpenAI(
            ...
            # (3)初始化情感识别链
            api.v1.chat.sentiment_chain = create_sentiment_chain(api.v1.chat.model)

            # （4）初始化模板prompt
            prompt_template = PromptTemplate.from_file(Config.PROMPT_TEMPLATE_PATH)
            api.v1.chat.prompt = ChatPromptTemplate.from_messages(
                [
            ...
            api.v1.chat.chain = api.v1.chat.prompt | ChatProcess.get_prompt | api.v1.chat.model
            # （6）构建带有消息历史的智能体
            api.v1.chat.with_message_history = RunnableWithMessageHistory(
                api.v1.chat.chain,
            ...
            knowledge_service = KnowledgeService(knowledge_store, Config.KNOWLEDGE_UPLOAD_DIR)
            knowledge_service.vectorstore = api.v1.chat.vectorstore
            queue = KnowledgeIngestionQueue(knowledge_store, knowledge_service)
            await queue.start()
            output_policy_store = OutputPolicyStore(Config.DB_PATH)
            self.state.vectorstore = api.v1.chat.vectorstore
            self.state.knowledge_store = knowledge_store
            self.state.knowledge_service = knowledge_service
            self.state.knowledge_queue = queue
            self.state.output_policy_store = output_policy_store
            print("应用初始化成功！")
