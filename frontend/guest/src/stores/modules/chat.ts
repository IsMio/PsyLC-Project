...
export const useChatStore = defineStore('chat', () => {
  ...
  const setChatMap = (id: string, data: ChatMessageVo[]) => {
    chatMap.value[id] = data?.map((item: ChatMessageVo) => {
      ...
      return {
        ...
        filtered: (item as any).filtered ?? false,
        filter_reason: (item as any).filter_reason ?? null,
        thinlCollapse: false,
      };
    });
  };
  ...
  function normalizeFilteredContent(content: string) {
    return content.replace(/^\[输出已按安全规范调整\]\s*/u, '').trim();
  }
