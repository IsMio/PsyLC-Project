...
export const useSessionStore = defineStore('session', () => {
  ...
  const createSessionList = async (data: Omit<CreateSessionDTO, 'id'>) => {
    ...
    try {
      ...
      const newSessionId = parseSessionId(res.data);
      if (!newSessionId)
        throw new Error('create_session returned invalid sessionId');
      // 创建会话后立刻查询列表会话
      // 1. 先找到被修改会话在 sessionList 中的索引（假设 sessionList 是按服务端排序的完整列表）
      const targetIndex = sessionList.value.findIndex(session => session.id === newSessionId);
